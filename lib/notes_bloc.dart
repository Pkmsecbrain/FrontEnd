...
class NotesBloc extends Bloc<NotesEvent, NotesState> {
  ...
  NotesBloc({required Box gBox}) : super(NotesInitial()) {
    on<StartListening>((event, emit) {
      // Di sini Anda dapat memulai pengenalan suara dan menghandle hasilnya
      // Misalnya, panggil metode untuk memulai pengenalan suara
      startListening();
      // Set state baru, misalnya OnListeningKey
      emit(OnListeningKey());
    });

    // ... definisi event-event lainnya
  }
  ...
  NotesBloc.emptyConstructor({required Box gBox}) : super(NotesInitial()) {
  ...
  String parseDate(String stringDate) {
    ...
    parsedDate =
        lang == 'en' ? parsedDate : "${translate[1]} ${translate[0].tr()}";
    return parsedDate;
  }
...
//----------------Fitur Listening------------------
  ...
  Future<void> speakContent(String content) async {
    await flutterTts.areLanguagesInstalled(["id-ID"]);
    await flutterTts.setLanguage("id-ID"); // Set bahasa TTS
    await flutterTts.setPitch(1.0); // Set pitch suara
    await flutterTts.speak(content); // Memainkan suara dari konten
  }

  void startListening() async {
    // Inisialisasi pustaka speech_to_text
    bool isAvailable = await _speech.initialize();

    // Mengecek apakah pengenalan suara didukung pada perangkat
    if (isAvailable) {
      _speech.listen(onResult: (result) {
        String spokenText = result.recognizedWords;
        // Panggil fungsi untuk mencocokkan suara dengan judul catatan di sini
        matchVoiceWithTitle(spokenText);
      });
    }
  }

  Future<List<String>> getAllNoteTitlesFromDatabase() async {
    // Implementasi untuk mendapatkan judul catatan dari database
    final List<Map<String, dynamic>> queryResult =
        await database.rawQuery('SELECT title FROM Notes');
    return queryResult.map((note) => note['title'] as String).toList();
  }

  // Fungsi untuk mencocokkan suara dengan judul catatan
  Future<void> matchVoiceWithTitle(String spokenText) async {
    // Di sini, Anda perlu mendapatkan daftar judul catatan dari database
    List<String> noteTitles = await getAllNoteTitlesFromDatabase();

    double highestSimilarity = 0;
    String matchedTitle = '';

    for (String title in noteTitles) {
      double similarity = StringSimilarity.compareTwoStrings(spokenText, title);
    ...
    if (highestSimilarity > 0) {
      emit(VoiceMatched(highestSimilarity, matchedTitle));
      String? content = await getContentByTitle(matchedTitle);
      if (content != null) {
        await speakContent(content); // Gunakan nilai content di sini
      }
    } else {
      // Handle jika tidak ada judul yang cocok dengan suara
    }
  }

// mendapatkan kontent berdasarkan tittle
  Future<String?> getContentByTitle(String title) async {
    ...
    if (queryResult.isNotEmpty) {
      String contentByTittle = queryResult.first['content'] as String;
      return contentByTittle;
    } else {
      return null;
    }
  }
